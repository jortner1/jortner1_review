...
public class RandomNumber {
         ...
         */
        public RandomNumber() {
                _currentSeed = System.currentTimeMillis() % MODULUS;
        }
        ...
        public RandomNumber(long seed) {
                _currentSeed = Math.abs(seed) % MODULUS;
        }
        ...
        public double nextDoubleRand() {
                long temp = MULTIPLIER*(_currentSeed%QUOT) - 
                                REM*(_currentSeed/QUOT);
                _currentSeed = (temp > 0) ? temp : temp + MODULUS;
                return (double) _currentSeed / (double) MODULUS;
        }
